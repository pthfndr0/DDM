...
country = {
	...
	DIPLOMAT_TRAVEL_TIME = 2,
	PROVINCE_OVERSEAS_PENALTY = 0.15, -- Each province req. this many goods flagged as overseas penalty 
...
military = {
	...
	COMBATLOSS_WAR_EXHAUSTION = 20, -- base war exhaustion in combat
	...
	NAVAL_COMBAT_DAMAGE_ORG_MULT = 0.2, -- scale the damage to ORG for balancing gameplay
	NAVAL_COMBAT_DAMAGE_STR_MULT = 0.8, -- scale the damage to STR for balancing gameplay
	NAVAL_COMBAT_DAMAGE_MULT_NO_ORG = 0, -- damage multiplier (to STR) when opponent has no ORG.
	...
	NAVAL_COMBAT_RETREAT_STR_ORG_LEVEL = 0.02, -- the retreating is available when STR or ORG % drops below this value (it's 0% to 100% value)
	...
	RECON_UNIT_RATIO = 0.05, -- the % of units in the army that must have a recon value to get the full bonus
	ENGINEER_UNIT_RATIO = 0.05, -- the % of units in the army that must have a fort attack value to get the full bonus
	SIEGE_BRIGADES_MIN = 2, -- the number of brigades needed for a siege to progress at normal speed
	SIEGE_BRIGADES_MAX = 8, -- the number of brigades above which you get no addition benefit in sieges
	SIEGE_BRIGADES_BONUS = 0.50, -- the bonus to siege speed from each brigade
